...
def get_load_path(root, load_run=-1, checkpoint=-1, run_name=''):
    ...
    print("root", root, "runs", runs)

    if load_run == -1 or load_run == '-1':
        if run_name != '':
            runs = [run_dir for run_dir in runs if run_name == run_dir.split('_')[-1]]
...
def update_cfg_from_args(env_cfg, cfg_train, args):
    ...
    if cfg_train is not None:
        ...
        if args.teacher_run_name is not None:
            cfg_train.rma.teacher_run_name = args.teacher_run_name
