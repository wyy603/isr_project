...
from rsl_rl.runners import RMARunner
from rsl_rl.runners import RMAAdaptationRunner
...
class TaskRegistry():
    ...
    def make_alg_runner(self, env, name=None, args=None, train_cfg=None, log_root="default") -> Tuple[OnPolicyRunner, LeggedRobotCfgPPO]:
        ...
        runner = runner_class(env, all_cfg, log_dir, device=args.rl_device, use_wandb=args.use_wandb)
        #save resume path before creating a new log_dir
        resume = train_cfg.runner.resume
        if resume:
            # load previously trained model
            resume_path = get_load_path(log_root, load_run=train_cfg.runner.load_run, checkpoint=train_cfg.runner.checkpoint, run_name=args.run_name)
